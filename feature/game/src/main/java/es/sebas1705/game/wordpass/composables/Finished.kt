...
/**
 * End of a Word-Pass game: stars and headline from the share of right answers, the points, and the
 * stats.
 *
...
fun Finished(
    ...
    // actualWord moves past the last answered word, so it counts the answers given.
    val answered = wordPassState.actualWord.coerceIn(wordPassState.correctAnswers, wordPassState.words.size.coerceAtLeast(wordPassState.correctAnswers))
    val wrong = answered - wordPassState.correctAnswers
    val ratio = if (answered > 0) wordPassState.correctAnswers / answered.toFloat() else 0f
    val scheme = MaterialTheme.colorScheme
    val stats = buildList {
        add(ResultStat(stringResource(R.string.feature_game_stat_correct), wordPassState.correctAnswers.toString(), gamePalette().success))
        ...
        if (wordPassState.mode == WordPassMode.SURVIVAL)
            add(ResultStat(stringResource(R.string.feature_game_stat_lives), wordPassState.lives.coerceAtLeast(0).toString(), scheme.tertiary))
    }
    GamePage(windowState, filled = false, verticalArrangement = Arrangement.Center) {
        GameResultContent(
            points = wordPassState.points,
            ratio = ratio,
            modeName = stringResource(wordPassState.mode?.strRes ?: es.sebas1705.core.resources.R.string.core_resources_any),
            stats = stats,
